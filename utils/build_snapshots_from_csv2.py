...
def linear_regression(y_values):
    ...
    Używane do rysowania kanału/flag na M5.
    """
    if not y_values or len(y_values) < 2:
        return None, None
    n = len(y_values)
...
# ============================================
# FLAGA / KANAŁ NA M5 (w kontekście impulsu H1)
# ============================================
...
def detect_flag_channel(
    ...
    """
    Wykrywanie kanału / flagi na M5:
    - używamy ostatnich max_bars świec,
    - regresja po high -> górna linia,
    - regresja po low  -> dolna linia,
    - szerokość kanału w ATR,
    - nachylenie zgodne z "flagą" wobec impulsu H1:
        impuls UP   -> flaga lekko w dół / płaska (slope <= 0),
        impuls DOWN -> flaga lekko w górę / płaska (slope >= 0).
    """
    if not m5_candles or impulse_direction not in ("UP", "DOWN"):
        return None
    if atr_ref is None or atr_ref <= 0:
        return None

    n_total = len(m5_candles)
    win_len = min(max_bars, n_total)
    if win_len < min_bars:
        return None

    window = m5_candles[n_total - win_len:]
    highs = [c["high"] for c in window]
    lows = [c["low"] for c in window]
    closes = [c["close"] for c in window]

    slope_high, intercept_high = linear_regression(highs)
    slope_low, intercept_low = linear_regression(lows)
    if slope_high is None or slope_low is None:
        return None

    x_last = win_len - 1
    upper = slope_high * x_last + intercept_high
    lower = slope_low * x_last + intercept_low
    width = upper - lower
    if width <= 0:
        return None

    width_atr = width / atr_ref
    if width_atr < min_width_atr or width_atr > max_width_atr:
        return None

    slope_mid = (slope_high + slope_low) / 2.0
    vertical_move = slope_mid * (win_len - 1)
    trend_atr = abs(vertical_move) / atr_ref
    if trend_atr > max_trend_atr:
        return None

    # orientacja wobec impulsu
    if impulse_direction == "UP" and slope_mid > 0:
        # kanał rosnący przy impulsie UP -> to bardziej trend continuation,
        # nie klasyczna flaga mean-reversion
        return None
    if impulse_direction == "DOWN" and slope_mid < 0:
        return None

    last_close = closes[-1]
    position = (last_close - lower) / width  # 0 = przy dole, 1 = przy górze
...
def build_snapshot_from_slices(daily_candles, hourly_candles, m5_candles, now_dt, instrument="BZ=F"):
    ...
    # --- M5 ---
    m5_part = {"candles_count": len(m5_candles)}
    if len(m5_candles) >= 1:
        closes_m5 = [c["close"] for c in m5_candles]
        highs_m5 = [c["high"] for c in m5_candles]
        lows_m5 = [c["low"] for c in m5_candles]

        rsi_5 = rsi(closes_m5, 14)
        atr_5 = atr(highs_m5, lows_m5, closes_m5, 14)
        ema20_5 = ema(closes_m5, 20)
        ema50_5 = ema(closes_m5, 50)

        last_idx_5 = len(m5_candles) - 1
        last_candle_5 = m5_candles[last_idx_5]
        last_close_5 = closes_m5[last_idx_5]
        last_rsi_5 = rsi_5[last_idx_5] if last_idx_5 < len(rsi_5) else None
        last_atr_5 = atr_5[last_idx_5] if last_idx_5 < len(atr_5) else None
        e20_5 = ema20_5[last_idx_5] if last_idx_5 < len(ema20_5) else None
        e50_5 = ema50_5[last_idx_5] if last_idx_5 < len(ema50_5) else None

        prev_rsi_5 = rsi_5[last_idx_5 - 1] if last_idx_5 - 1 >= 0 else None
        ...
        if last_idx_5 >= window:
            ...
            recent_high = max(highs_slice)
            recent_low = min(lows_slice)
        ...
        rsi5_sample = [v for i, v in enumerate(rsi_5) if v is not None and i <= last_idx_5]
        atr5_sample = [v for i, v in enumerate(atr_5) if v is not None and i <= last_idx_5]
        ...
        # --- FLAGA / KANAŁ NA M5 W KONTEKŚCIE IMPULSU H1 ---
        impulse_dir = h1_part.get("impulse_direction")
        atr_ref = h1_part.get("atr14")
        flag_info = detect_flag_channel(
            m5_candles,
            impulse_dir,
            atr_ref,
            max_bars=60,
        ...
        if flag_info:
            m5_part.update({
                "flag_active": True,
                "flag_impulse_direction": impulse_dir,
                "flag_upper": flag_info["upper"],
                "flag_lower": flag_info["lower"],
                "flag_width": flag_info["width"],
                "flag_width_atr": flag_info["width_atr"],
                "flag_slope": flag_info["slope"],
                "flag_age_bars": flag_info["age_bars"],
                "flag_position": flag_info["position"],
            })
        else:
            m5_part.update({
                "flag_active": False,
...
def main():
    ...
    parser.add_argument("--output", default="brent_snapshots_5m.csv")
    ...
    if args.days_back is not None and args.days_back > 0:
        ...
        cutoff_dt = last_dt - timedelta(days=args.days_back)
        cutoff_ts = cutoff_dt.strftime("%Y-%m-%dT%H:%M:%SZ")
        m5_all = [c for c in m5_all if c["ts"] >= cutoff_ts]

        if not m5_all:
            print(f"Po odfiltrowaniu do ostatnich {args.days_back} dni brak danych M5 – przerywam.")
            return

        first_dt = parse_ts_utc(m5_all[0]["ts"])
        print(
            f"Restricting to last {args.days_back} days: "
