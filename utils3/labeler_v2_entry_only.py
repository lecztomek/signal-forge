...
def generate_paths_dataset(
    ...
    """
    Generuje per-bar path dataset w trybie STREAMINGOWYM.

    Zamiast trzymać cały paths_df w pamięci:
      - buduje rekordy w małych chunkach,
      - każdy chunk od razu dopisuje do pliku CSV (append),
    ...
    # Jeżeli plik już istnieje – usuwamy, żeby nie dopisywać do starego
    ...
    for _, row in snapshots_with_entry.iterrows():
        ...
        # Jeśli nazbieraliśmy chunk – zapisujemy do CSV i czyścimy pamięć
    ...
    # Zapis ostatniego niedomkniętego chunka
    if records:
        chunk_df = pd.DataFrame.from_records(records)
        chunk_df.to_csv(
            out_paths_path,
...
def main():
    ...
    print("==> Generowanie per-bar path datasetu (streamingowo)...")
    total_rows = generate_paths_dataset(
        snapshots_with_entry=snapshots_with_entry,
        candles_5m=candles_5m,
        max_bars_ahead=max_bars_ahead,
        out_paths_path=out_paths_path,
        chunk_size=10_000,
    )
    print(f"  Powstało {total_rows} wierszy ścieżek.")
    print("  OK.")
