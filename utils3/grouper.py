...
def parse_args():
    ...
    p.add_argument(
        "--only-enrich",
        action="store_true",
        help="Jeśli ustawione: policz tylko featury + klastry i zapisz out-snapshots, "
             "bez liczenia statystyk grup i bez zapisu out-setups.",
    )
    return p.parse_args()
...
def main():
    ...
    # Tryb tylko wzbogacanie (do TEST / backtestu)
    if args.only_enrich:
        print("==> Tryb --only-enrich: pomijam liczenie statystyk grup wejściowych.")
        print("==> GOTOWE (out-snapshots zapisane).")
        return
